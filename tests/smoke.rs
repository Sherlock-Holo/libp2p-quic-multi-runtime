...
use libp2p_core::{
    multiaddr::Protocol,
    muxing::SubstreamBox,
    transport::{OrTransport, TransportError},
    upgrade,
};
...
#[test]
fn smol_smoke() {
    smol::block_on(smoke::<quic::smol::Provider>())
}
...
#[cfg(feature = "smol")]
#[test]
fn concurrent_connections_and_streams_smol() {
    let _ = tracing_subscriber::fmt()
        .with_env_filter(EnvFilter::from_default_env())
        .try_init();
    ...
    quickcheck::QuickCheck::new()
        .min_tests_passed(1)
        .quickcheck(prop::<quic::smol::Provider> as fn(_, _) -> _);
}
    ...
    fn block_on<R>(future: impl Future<Output = R> + Send, timeout: Duration) -> R {
        smol::block_on(async {
            ...
            match future::select(future, smol::Timer::after(timeout)).await {
                ...
                Either::Right(_) => panic!("future timed out after {timeout:?}"),
            }
        })
    }
}
